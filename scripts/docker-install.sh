...
echo "================================"
echo ""
# xray-info.txt 尾部自带"容器管理命令"段，下面的脚本输出也有同一段，先裁掉避免重复打印
INFO="$(docker exec "$CONTAINER_NAME" cat /etc/xray/xray-info.txt 2>/dev/null)" && printf '%s\n' "$INFO" | sed '/^容器管理命令:/,$d' || docker logs --tail 30 "$CONTAINER_NAME"
echo ""
